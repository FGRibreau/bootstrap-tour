...
(function($, window) {
  ...
  Tour = (function() {
    ...
    function Tour(options) {
      ...
      this._evt = $('<div/>');
      this.bind = $.proxy(this._evt.bind, this._evt);
      this.one = $.proxy(this._evt.one, this._evt);
    ...
    Tour.prototype.hideStep = function(i, e) {
      ...
      if (this._options.step.onHide !== step.onHide) {
        this._options.step.onHide(this, e);
      }
      if (step.reflex) {
    ...
    Tour.prototype.showStep = function(i, def) {
      ...
      if (this._options.step.onShow !== step.onShow) {
        defs.push(this._deferred(this._options.step.onShow(this, e)));
      }
      return $.when.apply($, defs).always(function() {
        ...
        if ($el.length === 0 || !$el.is(":visible")) {
          _this._evt.trigger(jQuery.Event("skipping", {
            step: step
          }));
        ...
        if (_this._options.step.onShown !== step.onShown) {
          _this._options.step.onShown(_this, e);
        }
        if (def) {
          return def.resolve();
        }
